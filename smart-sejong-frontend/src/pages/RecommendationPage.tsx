...
import {
  loadCompletedCourseInfo,
  saveCompletedCourseInfo,
  type CompletedCourseInfo,
...
export default function RecommendationPage() {
  ...
  const { data: timetableList, isLoading: timetableListLoading } = useQuery({
    queryKey: ['timetables'],
    queryFn: () => api.getTimetables(),
  })

  const saveMutation = useMutation({
    mutationFn: async (target: { mode: 'new'; name: string } | { mode: 'existing'; timetableId: number }) => {
      const targetId =
        target.mode === 'new'
          ? (await api.createTimetable({ name: target.name })).timetable_id
          : target.timetableId

      const sectionsToSave = [
        ...filters.fixedSections.map((s) => Number(s.sectionId)),
        ...(recommendations[selectedIdx]?.sections.map((s) => Number(s.sectionId)) ?? []),
      ].filter((id) => Number.isFinite(id) && id > 0)

      for (const sectionId of sectionsToSave) {
        await api.addSectionItem(targetId, sectionId)
      }

      for (const block of filters.customBlocks) {
        await api.addCustomItem(targetId, {
          name: block.title,
          day: block.day,
          start: minuteToTimeString(block.startMinute),
          end: minuteToTimeString(block.endMinute),
        })
      }

      return targetId
    },
    onSuccess: (timetableId) => {
      toast.success('시간표에 저장했습니다.')
      queryClient.invalidateQueries({ queryKey: ['timetables'] })
      queryClient.invalidateQueries({ queryKey: ['timetable', timetableId] })
      setShowSaveModal(false)
      navigate('/timetable')
    },
    onError: () => toast.error('시간표 저장에 실패했습니다.'),
  })
  ...
  const canSave =
    filters.fixedSections.length + filters.customBlocks.length + (selectedRec?.sections.length ?? 0) > 0
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {showSaveModal ? (
        <SaveToTimetableModal
          timetables={timetableList}
          isLoading={timetableListLoading}
          isSaving={saveMutation.isPending}
          onClose={() => setShowSaveModal(false)}
          onSave={(target) => saveMutation.mutate(target)}
        />
      ) : null}
    </div>
  )
